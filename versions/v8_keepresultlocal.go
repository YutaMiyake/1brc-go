...
func readFileLineByLineIntoAMap(filepath string) (*swiss.Map[mapkey, *cityTemperatureInfo], error) {
	...
	for i := 0; i < runtime.NumCPU(); i++ {
		wg.Add(1)
		go func() {
			m := swiss.NewMap[mapkey, *cityTemperatureInfo](maxcity)
			for chunk := range chunkStream {
				processReadChunk(m, chunk)
			}
			resultStream <- m
			wg.Done()
		}()
...
func processReadChunk(result *swiss.Map[mapkey, *cityTemperatureInfo], buf []byte) {
		...
		case '\n':
			if (index-start) > 1 && hash != 0 {
				...
				if val, ok := result.Get(hash); ok {
					...
					result.Put(hash, &cityTemperatureInfo{
