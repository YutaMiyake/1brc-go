...
func evaluate(input string) string {
	...
	var count int
	mapOfTemp.Iter(func(_ uint32, info *cityTemperatureInfo) bool {
...
var mapPool = sync.Pool{
	New: func() interface{} {
		return swiss.NewMap[uint32, *cityTemperatureInfo](500)
	},
}

func readFileLineByLineIntoAMap(filepath string) (*swiss.Map[uint32, *cityTemperatureInfo], error) {
	...
	mapOfTemp := swiss.NewMap[uint32, *cityTemperatureInfo](maxcity)
	resultStream := make(chan *swiss.Map[uint32, *cityTemperatureInfo], 10)
	...
	for t := range resultStream {
		t.Iter(func(hash uint32, tempInfo *cityTemperatureInfo) bool {
...
func processReadChunk(buf []byte, resultStream chan<- *swiss.Map[uint32, *cityTemperatureInfo]) {
	toSend := mapPool.Get().(*swiss.Map[uint32, *cityTemperatureInfo])
	var start int
	var hash uint32
	...
	for index, char := range buf {
		...
		case ';':
			keyStart = start
			keyEnd = index
			hash = mmh3.Hash32(buf[start:index])
			start = index + 1
		case '\n':
